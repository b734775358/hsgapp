<template>
	<view class="box">
		<view class="fixed-top_box">
			<uni-nav-bar statusBar shadow leftIcon="arrowleft" rightIcon="plusempty" title="对外发文" @clickRight="submit" @clickLeft="goBack"/>
...
<script>
	...
	export default {
		...
		methods: {
			...
			submit() {
				uni.navigateTo({
					url: '/pages/contract/salesSupplementaryAgreement/index'
				});
			},
			// 切换tab
			changeTab(index) {
			...
			getList() {
				...
				that.$http(`/b2b/order_project/list_data.jhtml`, {
					...
					// hasStamp:'', 
					...this.paginationData,
					// sn: this.searchValue!=null?this.searchValue:'RF',
